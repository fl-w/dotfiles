### 1.5.0: 03 May 2019
* Update to version 1.10.0 of `js-beautify`
  * Adds `indent_empty_lines` and `templating` settings.
* Bring back schema for config in VS Code settings. (Major maintenance burden due to VS Code not handling local file $refs correctly)

### 1.4.11: 20 Mar 2019
* Stop setting formatter when no selector matches.
